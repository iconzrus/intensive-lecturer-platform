...
public String toString() { return "User{id=" + id + "}"; }`,
    ...
    topic({
      ...
      explainBrief: [
        ...
        '`hashCode` выбирает bucket в `HashMap`; внутри bucket совпадение ключа проверяют через `equals`.',
        'Если контракт нарушен, коллекции начинают вести себя странно: дубликаты, потери элементов, ошибки поиска.',
        'Mutable-ключ после `put` может «потеряться», если поле участвует в `hashCode`/`equals`.',
  ...
  return super.clone();
    ...
    topic({
      ...
      explainBrief: [
        ...
        'LSP-ловушка: обязательный `fly()` у всех «птиц» — лучше интерфейс `Flyable`, а не `UnsupportedOperationException` в `Penguin`.',
      ],
      interviewFocus: [
        {
...
service.pay(invoice);`,
    ...
    topic({
      ...
      explainBrief: [
        ...
        'Композиция (Order→OrderItem) — сильное владение; агрегация (Team→Employee) — часть может жить отдельно.',
    ...
    topic({
      ...
      interviewFocus: [
        ...
        },
        {
          ...
          expectedAnswer:
            'Когда профилирование показало много дублей в heap и понятная выгода: `intern()` кладёт строку в String Pool (в heap). Без нужды может раздуть pool и удерживать память — без замеров не применяют.',
        },
      ],
      codeExample: {
