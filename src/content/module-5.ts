...
export const module5: LectureModule = {
  ...
  topics: [
    topic({
      id: 'final-prep-plan',
      title: 'Финальное тестирование: план подготовки и покрытие банка вопросов',
      simpleDefinitionOverride:
        'Тема задаёт структуру: какие блоки учим, в каком порядке и как проверяем, что готовы к тесту без прямого заучивания формулировок.',
      quickAnswer:
        'Идём по 8 теоретическим блокам: exceptions+I/O, concurrency, Maven+Git, patterns+SOLID, network+web, Core Java, collections+algorithms, Stream API; после каждого блока проверяем себя в формате «вопрос -> короткий ответ».',
      explainBrief: [
        'Цель — закрыть все 30 тестовых пунктов через понимание тем, а не через запоминание вариантов ответа.',
        'После каждого блока делаем мини-самопроверку: 3-5 устных вопросов и один короткий код/схема.',
        'Если студент путается в терминах, сначала словарь, затем примеры и только потом контрольные вопросы.',
        'Контрольный вопрос: когда блок считается закрытым? (Когда термин объясняется своими словами и подтверждается примером).',
      ],
      glossary: [
        {
          term: 'Покрытие темы',
          meaning: 'Состояние, когда студент может объяснить идею, дать пример и не путает соседние понятия.',
        },
        {
          term: 'Мини-самопроверка',
          meaning: 'Короткий цикл из устных вопросов и одного маленького практического фрагмента.',
        },
        {
          term: 'Пробел',
          meaning: 'Конкретный участок темы, где студент не может уверенно объяснить «что это» и «когда применять».',
        },
      ],
      interviewFocus: [
        {
          question: 'Как понять, что блок закрыт перед тестом?',
          expectedAnswer:
            'Студент объясняет понятие своими словами, даёт корректный пример и не путает близкие сущности.',
        },
      ],
      codeExample: {
        title: 'Матрица подготовки (8 блоков)',
        language: 'text',
        snippet: `1) Exceptions + I/O
2) Threads + java.util.concurrent
3) Maven + Git
4) Patterns + SOLID
5) OSI + HTTP + Tomcat + Servlet lifecycle
6) ClassLoader + interfaces + enum + inheritance
7) Collections + complexity
8) Stream API`,
        walkthrough: [
          'Проходите блоки последовательно, фиксируя пробелы после каждого.',
          'Пробелы закрывайте теорией и одним коротким практическим примером.',
        ],
        commonPitfall: 'Пытаться запомнить ответы «буквами», не понимая терминов и контекстов.',
      },
...
git stash`,
    ...
    topic({
      ...
      explainBrief: [
        'Builder помогает собирать сложные объекты с читаемыми цепочками вызовов.',
        'Decorator расширяет функциональность без изменения исходного класса.',
        'DIP: high-level модули не должны зависеть от concrete-реализаций.',
        'Контрольный вопрос: какой паттерн чаще связан с вложенным static class? (Builder).',
        'Контрольный вопрос: какой принцип SOLID про абстракции? (DIP).',
      ],
      glossary: [
    ...
    PaymentService(PaymentGateway gateway) {
        ...
        commonPitfall: 'Жёстко new-ить конкретный dependency внутри сервиса.',
      },
      usefulLinksOverride: MODULE5_LINKS,
      estimatedMinutes: 7,
    }),
